...
def main():
  ...
  bo_boston = BayesianOptimization(f=optimize_boston_bo_function,                     # Objective function       
  ...
  try:
    ...
    f.close()
    bo_boston.run_optimization(max_iter=max_iter, eps=1e-20)
    f = open("boston_bo_hyperparams.txt", "a")
    ...
    print("Error")
  # Print results
  result = "BO error --> " + str(bo_boston.fx_opt) + " with hyperparams: cols = " + cols_value_to_cols(bo_boston.x_opt[0]) + ", rows = " + rows_value_to_rows(bo_boston.x_opt[1]) + ", threshold = " + str(bo_boston.x_opt[2]) + ", num_instances = " + str(int(bo_boston.x_opt[3])) + "."
  print(result)
  return result
