...
def optimize_classification_bo_function(params):
  ...
  f = open("classification_bo_hyperparams.txt", "a")
...
def main():
  ...
  bo_classification = BayesianOptimization(f=optimize_classification_bo_function,                     # Objective function       
  ...
  try:
    ...
    f.close()
    bo_classification.run_optimization(max_iter=max_iter, eps=1e-20)
    f = open("classification_bo_hyperparams.txt", "a")
  ...
  # Print results
  result = "BO error --> " + str(bo_classification.fx_opt) + " with hyperparams: Threshold = " + str(bo_classification.x_opt[0]) + ", Min_instances_slice = " + str(bo_classification.x_opt[1]) + ", Min_features_slice = " + str(bo_classification.x_opt[2]) + ", Rows = " + rows_value_to_rows(bo_classification.x_opt[3]) + "."
  print(result)
  return result
