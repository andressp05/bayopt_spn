...
def optimize_mnist_bo_function(params):
  # Carga Dataset MNIST
  olivetti_data, olivetti_target = fetch_olivetti_faces(return_X_y=True, shuffle=True)
  
  f = open("olivetti_bo_hyperparams.txt", "a")
  ...
  for i in range(k):
    ...
    x_train, x_test, y_train, y_test = divide_k_folds(k, olivetti_data, olivetti_target, i)
    # Prepare Train Data
    ...
    try:
      spn_classification = learn_classifier(train_data,
                          ...
                          , Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian
                          , Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian,
                          ...
                          Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian
                          , Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian
                          , Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian, Gaussian]).add_domains(train_data),
                          learn_parametric, 4096, **hyperparams)
      ...
      # Calculate Error
      y_predict = np.hsplit(predict_data,[4096])
...
def main():
  ...
  bo_olivetti = BayesianOptimization(f=optimize_mnist_bo_function,                     # Objective function       
  ...
  try:
    ...
    f.close()
    bo_olivetti.run_optimization(max_iter=max_iter, eps=1e-20)
    f = open("olivetti_bo_hyperparams.txt", "a")
    ...
    print("Error")
  # Print results
  result = "BO error --> " + str(bo_olivetti.fx_opt) + " with hyperparams: cols = " + cols_value_to_cols(bo_olivetti.x_opt[0]) + ", rows = " + rows_value_to_rows(bo_olivetti.x_opt[1]) + ", threshold = " + str(bo_olivetti.x_opt[2]) + ", num_instances = " + str(int(bo_olivetti.x_opt[3])) + "."
  print(result)
  return result
